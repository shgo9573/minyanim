...
app.get('/minyan', async (req, res) => {
    ...
    try {
        ...
        let weekdayMinyanim = []; // מנייני חול (Mark = true)
        let shabbatMinyanim = []; // מנייני שבת/עש"ק/מוצ"ש (Mark = false)
        ...
        if (lastMove === '3') {
            ...
            if (history.length === 0) {
                if (isShabbatMode) {
                    prefix = isTomorrow ? "תפילות השבת הבאות הן: " : "המניין הבא לשבת הוא: ";
                } else {
